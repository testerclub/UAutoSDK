...
namespace TcpServer
{
    ...
    public class MsgProtocol : ProtoFilter
    {
        /* 简单协议过滤器
		...
		*/
        ...
        public void input(byte[] data)
        {
            ...
            //while (buf.Length > 0)
            //{
            //    int n = Array.LastIndexOf(buf, 38);
            //    if (n > 0)
            //    {
            //        byte[] data_body = Slice(buf, 0, n);
            //        string content = System.Text.Encoding.Default.GetString(data_body);
            //        msgs.Add(content);
            //        buf = Slice(buf, n, buf.Length);
            //    }
            //    else
            //    {
            //        break;
        ...
        {
            //var response = System.Text.Encoding.Default.GetBytes(string.Format("altstart::{0}::altLog::::altend", content));
            //return response;
            return System.Text.Encoding.Default.GetBytes(string.Format("altstart::{0}::altLog::::altend", content));
        }

        public byte[] pack2(String content)
        {
    ...
    public class AsyncTcpServer : IDisposable
    {
        ...
        //private KeyValuePair<string, TcpClientState> _client;
        private TcpClientState _client;
        ...
        public AsyncTcpServer(IPAddress localIPAddress, int listenPort)
        {
            ...
            //_client = new KeyValuePair<string, TcpClientState>();
        ...
        public AsyncTcpServer Stop()
        {
            ...
            try
            {
                ...
                //}
                //_clients.Clear();
                //if(_client.Value != null && _client.Value.TcpClient != null)
                //    _client.Value.TcpClient.Client.Disconnect(false);
                //_client = new KeyValuePair<string, TcpClientState>();
                if(_client != null)
                {
                    _client.TcpClient.Client.Disconnect(false);
                }
                _client = null;
            }
        ...
        private void HandleTcpClientAccepted(IAsyncResult ar)
        {
            ...
            MsgProtocol prot = new MsgProtocol();
            ...
            //});
            //_client = new KeyValuePair<string, TcpClientState>(tcpClientKey, internalClient);
            ...
            //如果当前连接存在,停止监听
            //if (_client.Key != null || _client.Key != "")
            //{
            //    if (_client.Value != null && _client.Value.TcpClient != null && _client.Value.TcpClient.Connected)
            //    {
            //        tcpListener.Stop();
            //        return;
            //    }
        ...
        public void SendToAll(byte[] datagram)
        {
            ...
            //}
            //if(_client.Key != null && _client.Key != "" && _client.Value != null)
            //{
            //    Send(_client.Value.TcpClient, datagram);
            //}
            if(_client != null)
            {
                Send(_client.TcpClient, datagram);
            }
        }
